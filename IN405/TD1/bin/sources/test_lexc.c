...
int main(int argc, char** argv)
{
	...
	struct lexc* lexc = lexc_init("no name", 1);

	short condition_finish = 1;
	while(condition_finish)
	{
		char cmd[CMD_BUFFER];

		printf("[%s]$ ", argv[0]);
		fgets(cmd, CMD_BUFFER, stdin);
        cmd[strlen(cmd) - 1] = '\0';
        
        struct cmd com = cmd_init(cmd);
        
        if(cmd_interpret(&vector, &lexc, com))
        {
        	condition_finish = 0;
        }
		cmd_fini(&com);
...
/*vector_add(vector, "bite", 1);
vector_add(vector, "arthur", 1);

vector_render(vector);
printf("%d indice\n", vector_check(vector, "arthur"));

vector_remove(vector, "arthur");
vector_render(vector);
